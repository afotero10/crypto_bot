...
#closes=[3800]
#closes=pd.DataFrame(df_datos_iniciales['Close'])
...
def cantidad_venta(symbol,side,precio):
    ...
    if my_file.is_file():
        ...
        lista = lista.filter(['orderId', 'time', 'price', 'qty', 'symbol', 'side'], axis=1)
 ...
 if is_candle_closed:
      ...
      if len(df_datos)>RSI_PERIOD:
             ...
             if len(df_datos)>BB_PERIOD:
                 ...
                 bb_up_pasado=df_datos.loc[df_datos.index[-2],'BB_upper']
                 bb_up_actual = df_datos.loc[df_datos.index[-1], 'BB_upper']
                 bb_low_pasado = df_datos.loc[df_datos.index[-2], 'BB_lower']
                 bb_low_actual = df_datos.loc[df_datos.index[-1], 'BB_lower']
                 dt_actual=df_datos.index[-1]
                 #print('tail ',df_datos.tail())
             print('rsi: ',rsi_actual)
             print('time: ', dt_actual)
             #ss=client.get_asset_balance(asset='USDT')
             #print('wallet',ss['free'])

             #venta
             if rsi_actual>RSI_top:#and precio_pasado<bb_up_pasado and precio_actual>bb_up_actual:

                #print('datos para venta')
                #print(df_datos.tail())
                if num_open_orders(open_orders,'SELL')<4and cuenta_ventas<4 :

                    quan_sell=cantidad_venta(symbol, 'BUY', precio_actual)
